...
mutation DynamicIslandLinearReply($input: CommentCreateInput!) {
...
function readOAuthSession() {
  const oauth = asObject(DynamicIsland.store.get("oauth"));
...
function readStringArray(key) {
  const stored = DynamicIsland.store.get(key);
...
function writeStringArray(key, values, limit) {
  ...
  DynamicIsland.store.set(key, deduped);
}
...
function resetBaselineForToken(signature) {
  DynamicIsland.store.set("linearTokenSignature", signature);
  DynamicIsland.store.set("linearBaselineReady", false);
  DynamicIsland.store.set("seenMentionNotificationIDs", []);
  DynamicIsland.store.set("pushedMentionKeys", []);
}
...
async function graphqlRequest(query, variables) {
  ...
  const response = await DynamicIsland.http.fetch(LINEAR_API_URL, {
    method: "POST",
    headers: {
...
async function pollMentions(force) {
  ...
  if (!force && now - lastPollStartedAt < MIN_POLL_GAP_MS) return;

  const oauth = oauthSessionState();
  const accessToken = oauth.connected && oauth.session ? oauth.session.accessToken : "";
  const signature = tokenSignature(accessToken);
  const storedSignature = normalizeText(DynamicIsland.store.get("linearTokenSignature"));
  ...
  try {
    ...
    const baselineReady = DynamicIsland.store.get("linearBaselineReady") === true;

    if (!baselineReady) {
      DynamicIsland.store.set("linearBaselineReady", true);
      ...
      if (unseenMentions.length > 0) {
        ...
        if (settingBoolean("autoReveal", true) && typeof DynamicIsland.island.activate === "function") {
          DynamicIsland.island.activate(true);
        }
      } else {
        persistPushedMentionKeys(allMentions, pushedMentionKeys);
      }
...
async function submitReply(body) {
  ...
  if (replyComposer.notificationSourceID && typeof DynamicIsland.system.dismissNotification === "function") {
    DynamicIsland.system.dismissNotification(replyComposer.notificationSourceID);
  }
...
function fullExpandedView() {
  ...
  if (!oauth.connected) {
    ...
    return View.vstack([
      View.text("Linear Mentions", { style: "title", lineLimit: 1 }),
      View.text(oauth.expired ? "Your saved Linear session expired. Start OAuth again." : "Authorize Linear to receive mentions in the Dynamic Island.", {
        style: "caption",
        color: "gray",
        lineLimit: 2
      }),
...
DynamicIsland.registerModule({
  ...
  onAction(actionID) {
    if (actionID === "login-linear") {
      DynamicIsland.openURL(LINEAR_AUTHORIZE_URL);
      return;
    }

    if (actionID === "disconnect-linear") {
      DynamicIsland.store.set("oauth", null);
      DynamicIsland.store.set("linearBaselineReady", false);
      DynamicIsland.store.set("linearTokenSignature", "");
      DynamicIsland.store.set("seenMentionNotificationIDs", []);
      DynamicIsland.store.set("pushedMentionKeys", []);
      closeReplyComposer();
      state = {
    ...
    if (actionID === "open-in-linear" && replyComposer) {
      ...
      if (targetURL) {
        DynamicIsland.openURL(targetURL);
      }
      return;
    }
